...
The parser is a PHP **library script, not an executable**. It is invoked as an
argument to a PHP binary:

```bash
analysis/scripts/run-php.sh analysis/scripts/extract-pdflib-calls.php \
  <repo-path> --methods <reflected-methods-file> [--json-out <path>]
```

The parser needs no PHP extensions — only `nikic/php-parser` — so `run-php.sh`
runs it on host PHP when available and falls back to the base image otherwise.
Do not use `--image` for the parser; that flag exists for `probe.php`, which
must reflect the PDFlib extension and can only do so inside the image.

A host/container PHP version mismatch is not a concern here: php-parser has its
own lexer and parses target syntax independently of the runtime executing it.
Set the parser's target version explicitly rather than inheriting whatever the
host happens to run.

Do **not** give it a shebang line and do **not** make it executable. The `.sh`
scripts alongside it carry `+x` because they are invoked directly; the parser is
not. Going through `run-php.sh` keeps the host/image fallback working, and keeps
the invocation identical whether or not the machine has PHP installed.

**Write all paths in the parser as given on the command line**, and resolve them
relative to the current working directory rather than assuming a container mount
point. The same script has to work from the host and from `/work` inside the
image.
