...
`references/findings-schema.md`, `references/environment.md`, and
...
Cheap, but **not uniform** — see `references/environment.md`. Repos split across
three variants: `php-apache-pdflib` (8), `php-apache-pdflib-full` (6), and
`php-cli-pdflib` (1). The PDFlib binary is the same 10.0.1 across all three, so
the API surface is identical; the variants differ in web server and extras.
Record which one this repo pins.
...
- Which of the three pdflib variants this repo resolves to
- **Follow transitive inheritance.** A `FROM` pointing at another app repo
  rather than a pdflib image is legitimate, not an anomaly —
  `pcw-ppe-imagetool-etl` inherits through `pcw-ppe-datamart-etl`. Walk the
  chain, record each hop, and record the variant it ultimately resolves to.
  Never report "no pdflib base found" without following the chain first.
- Whether the variant matches what `references/environment.md` expects for this
